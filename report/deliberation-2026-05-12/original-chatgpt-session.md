# Original ChatGPT Development Session — Historical Nanochat

- **Source URL**: https://chatgpt.com/c/69dc25a0-7f50-83e8-b46f-9be4018ccbb4
- **Title**: Historical Nanochat Training
- **Messages**: 5
- **Captured**: 2026-05-14
- **Conversation ID**: 69dc25a0-7f50-83e8-b46f-9be4018ccbb4 (note: ID prefix predates current deliberation IDs by ~3 weeks — likely Jan/Feb 2026)

This is the original conversation in which the historical-nanochat project was conceptualized. Retrieved via Chrome DevTools Protocol from the ChatGPT browser session on requiem (2026-05-14).
...
The architecture itself is broadly sensible. RoPE is standard and solid; QK-normalization exists specifically to make attention less prone to saturation; and GQA’s main documented advantage is inference efficiency, where it gets quality close to MHA with much faster decoder-time KV handling. None of those choices are red flags. 
arXiv
+1

For your setup, d12 / 125M is a good science model, even if it is not the model that best matches your total 16B-token reservoir. Put differently: for your available data, 125M is small; for your available compute, 125M is about right. Nanochat itself is organized around depth as the main scale knob, and its current defaults describe a tokens:params regime around 10.5 as compute-optimal, with nearby tradeoffs used in practice. 
GitHub
+2
GitHub
+2
...
Subword robustness is a real issue. Recent work found that subword tokenization is sensitive to typos and format variation, and that BPE-dropout/subword regularization can help. That is especially relevant for OCR-heavy newspapers. I would seriously consider training the tokenizer with a little regularization, or at least testing it in the proxy stage. 
arXiv

A more exploratory option is to try something beyond vanilla BPE. There is recent work on Length-MAX tokenization claiming materially fewer tokens than BPE at the same vocab size. I would treat that as experimental, not your main branch, but historical corpora are exactly the kind of place where token efficiency might matter more than usual. 
arXiv

4) RTX 3090 optimization

Yes, torch.compile is worth retrying on native Linux. Current PyTorch documentation is explicit that torch.compile is meant to speed up PyTorch code with minimal changes, and that graph breaks usually mean lost optimization opportunities rather than silent corruption. On your current stack, I would run a controlled A/B benchmark for a few hundred steps. 
PyTorch Documentation

Also verify that you are actually on PyTorch’s optimized SDPA path. The current docs say scaled_dot_product_attention can use FlashAttention-2 or memory-efficient attention kernels on CUDA and will automatically choose the best implementation it can. If your code path is bypassing that, you may be leaving easy throughput on the table. 
PyTorch Documentation

If batch size 4 only barely OOMs, the most practical tool is activation checkpointing. PyTorch is very clear that checkpointing trades compute for memory, and it recommends use_reentrant=False. On a 3090, that trade is often worth it if it unlocks a better device batch or a deeper model. 
PyTorch Documentation

For mixed precision, benchmark both modes your stack supports cleanly. PyTorch’s AMP docs note that fp16 can underflow or overflow depending on the model, so if you stay on fp16, use proper gradient scaling and watch for NaNs. 
PyTorch Documentation

Also enable TF32 matmuls on Ampere unless you have evidence that convergence regresses. PyTorch’s numerical-accuracy note explicitly recommends TF32 for many neural workloads when full fp32 precision is not necessary. 
PyTorch Documentation
...
The “closest idea” space now includes at least two strands. One is the historical-simulation critique that argues period pretraining may be necessary. The other is Ranke-4B, which is pursuing large time-locked historical models trained from scratch and explicitly measuring pre- vs post-cutoff knowledge and normative views. 
arXiv
+2
GitHub
+2
...
True historical pretraining vs modern-model fine-tuning on the same benchmark. The recent historical simulation paper makes this especially compelling. 
arXiv

OCR-cleaning ablation: raw OCR vs aggressive filtering vs section-level filtering vs curriculum.

Source-mixture ablation: newspapers vs books vs science vs law, and measure not only BPB but worldview shifts.

Tokenizer ablation: 32K vs 48K/64K, BPE vs unigram/SentencePiece, maybe one experimental Length-MAX branch. 
arXiv
+2
arXiv
+2

Cutoff-boundary study: 1900, 1913, 1929 as separate checkpoints, then plot leakage curves and worldview drift.

Human-vs-automatic authenticity evaluation: especially valuable because the literature already suggests automatic judges can overestimate authenticity. 
arXiv
+1
...
Freeze the data pipeline with immutable manifests, token counts, and run metadata.

Add section-level OCR filtering and span-level dedup, not just document-level filters.

Build the benchmark first: cutoff fidelity, ex-ante reasoning, authenticity, worldview, leakage.

Run a clean d8/d12/d16 series before any d20 ambition.

Ablate tokenizer size and learn source weights with proxy models instead of hand-tuning them.

Only then decide whether a bigger cloud run is justified.
